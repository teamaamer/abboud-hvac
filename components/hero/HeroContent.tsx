...
export default function HeroContent({ onEstimateClick }: HeroContentProps) {
  return (
    <div className="relative z-10">
      <div className="inline-flex items-center gap-2 bg-[var(--cyan-500)]/10 backdrop-blur-sm border border-[var(--cyan-500)]/30 rounded-full px-4 py-2 mb-3 lg:mb-4 animate-slide-in-down opacity-0" style={{ animationDelay: '0.1s' }}>
        <Sparkles className="h-4 w-4 text-[var(--cyan-400)]" />
        <span className="text-sm font-semibold text-[var(--cyan-400)]">Trusted by 500+ Happy Customers</span>
      </div>
      
      <h1 className="text-3xl sm:text-4xl lg:text-4xl xl:text-5xl font-bold mb-3 lg:mb-4 leading-tight bg-gradient-to-r from-white via-white to-[var(--cyan-400)] bg-clip-text text-transparent animate-fade-in-up opacity-0" style={{ fontFamily: 'var(--font-heading)', animationDelay: '0.2s' }}>
        Expert HVAC & Electrical Services You Can Trust
      </h1>
      ...
      <p className="text-base sm:text-lg lg:text-xl mb-4 lg:mb-5 text-gray-200 animate-fade-in-up opacity-0" style={{ animationDelay: '0.4s' }}>
        Licensed, insured technicians providing fast, reliable service for your home or business. Same/next-day appointments available.
      </p>
